...
The following scenarios describe events when reading `EBML Documents` and the REQUIRED behavior by an `EBML Reader`.

If a `Master Element` contains a `CRC-32 Element` that doesn't validate, then the `EBML Reader` MUST ignore all contained data except for `Descendant Elements` which contain their own valid `CRC-32 Element`.

If a `Master Element` contains more occurrences of a `Child Master Element` than permitted according to the `maxOccurs` and `recurring` attributes of the definition of that `Element` then the occurrences in addition to `maxOccurs` MUST be ignored.

If a `Master Element` contains more occurrences of a `Child Element` that is not a `Master Element` than permitted according to the `maxOccurs` attribute of the definition of that `Element` then occurrences in addition to `maxOccurs` MUST be ignored.
